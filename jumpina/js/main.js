document.addEventListener('DOMContentLoaded', () => {
    ...
    let gameState = {
        ...
        level: {
            ...
            grid: [],
            playGrid: [], // A copy of the grid for the current play session
        },
        collectibles: {
            total: 0,
            collected: 0,
    ...
    function draw() {
        ...
        for (let y = 0; y < gameState.level.height; y++) {
            for (let x = 0; x < gameState.level.width; x++) {
                let tileType = gameState.gameMode === 'PLAY' ? gameState.level.playGrid[y][x] : gameState.level.grid[y][x];
    ...
    function checkCollisionsY() {
        ...
        for (let row = startRow; row <= endRow; row++) {
            for (let col = startCol; col <= endCol; col++) {
                 ...
                 const tile = gameState.level.playGrid[row][col];
                 ...
                 if (tile && isColliding(p, tileX, tileY)) {
                     if (tile === 'platform' || tile === 'ground') {
                        ...
                        gameState.level.playGrid[row][col] = null; // Remove coin from play grid
    ...
    function createDefaultLevel() {
        ...
        // Clear the grid first, except for the ground layer
    ...
    function init() {
        ...
        function switchToPlayMode() {
            ...
            let playerStart = { x: 100, y: 100 }; // Fallback

            // Create a fresh copy of the level for gameplay
            ...
            // If no coins, activate exit immediately
            if (gameState.collectibles.total === 0) {
                gameState.exit.activated = true;
            }
        ...
        // Start in Play mode with the default level
        switchToPlayMode();
